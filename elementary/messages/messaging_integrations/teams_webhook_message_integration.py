...
def send_adaptive_card(webhook_url: str, card: dict) -> requests.Response:
    ...
    # Wrap the card in the expected Teams webhook payload format
    payload = {
        "type": "message",
        "attachments": [
            {
    ...
    if response.status_code == 202:
        # a request has been accepted for processing, but processing has not been completed or may not have started
        logger.debug("Got 202 response from Teams webhook, assuming success")
    return response


class TeamsWebhookMessageIntegration(
